...
'''
def get_df(filename,percentile):
    ...
    data["reduced_distance"] = data["distance"]*(1/np.percentile(data["distance"], percentile)) #assumes less than 1% of atoms have evaporated off surface
...
def get_Comp_by_atom_and_abs_dist(dataframe, num_bins, atoms_per_bin):
    ...
    for i in range(num_bins):
        ...
        #x.append(RMC_temp_red["reduced_distance"].iloc[-1])
...
def get_Chi_Squared(filename):
    ...
    NEEDS TO BE FIXED
    '''
    #all_X2s = re.findall(fr"(?m)^{str_to_search}\s+(-?\d+\.?\d*)", file_text)
    #all_X2s = re.findall("s2x2:\s+(-?\d+\.?\d\d*)", file_text)
